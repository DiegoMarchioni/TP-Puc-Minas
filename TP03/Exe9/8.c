...
void clone(struct Jogador *self, struct Jogador J) {
    self->id = J.id;
    strcpy(self->cidadeNascimento, J.cidadeNascimento);
    strcpy(self->estadoNascimento, J.estadoNascimento);
    strcpy(self->nome, J.nome);
    self->altura = J.altura;
    self->peso = J.peso;
    self->anoNascimento = J.anoNascimento;
    strcpy(self->universidade, J.universidade);
}
...
int main(int argc, char **argv)
{
    ...
    FILE *matricula;
    matricula = fopen("804762_quicksort2.txt", "w+");

    fprintf(matricula, "804762\t %d \t %d", comp, mov);
