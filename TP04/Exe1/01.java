...
public class Arv {
    ...
    public static void main(String[] args){
        ...
        for(int i = 0; i < numEntrada2; i++){
            arvore.pesquisarJogador(entrada3[i]);
        }
...
class arvoreBinaria {
    ...
    public arvoreBinaria(Jogador jogador){
        raiz = new No(jogador);
    }

    public void inserirJogador(Jogador jogador){
        raiz = inserirJogador(jogador, raiz);
    }

    public No inserirJogador(Jogador elemento, No no){
        if(no == null) no = new No(elemento);
        else if(elemento.nome.compareTo(no.elemento.nome) < 0) no.esq = inserirJogador(elemento, no.esq);
        else if(elemento.nome.compareTo(no.elemento.nome) > 0) no.dir = inserirJogador(elemento, no.dir);
        return no;
    ...
    public void pesquisarJogador(String nome){
        System.out.printf(nome + " raiz");
        raiz = pesquisarJogador(nome, raiz);
        MyIO.println("");
    }

    public No pesquisarJogador(String nome, No no){
        if(no == null){
            System.out.printf(" NAO");
            comp ++;
        }
        else if(nome.compareTo(no.elemento.nome) < 0){
            MyIO.print(" esq");
            comp ++;
            no.esq = pesquisarJogador(nome, no.esq);
        }
        else if(nome.compareTo(no.elemento.nome) > 0){
            MyIO.print(" dir");
            comp ++;
            no.dir = pesquisarJogador(nome, no.dir);
        }
        else MyIO.print(" SIM");
        return no;
    }
    ...
    public void caminhamentoCentral(No no) {
        if (no != null) {
            caminhamentoCentral(no.esq); // Visitar a subárvore esquerda
            System.out.println(no.elemento.nome + " "); // Visitar o nó atual
            caminhamentoCentral(no.dir); // Visitar a subárvore direita
        }
    }
